...
const register = async (req, res) => {
    try {
        ...
        switch (role) {
            case 'Admin':
                relatedDocument = await Admin.create({ email, password: hashedPassword });
                newUser.admin_data = relatedDocument._id;
                break;
            case 'Doctor':
                relatedDocument = await Doctor.create({ email, password: hashedPassword, name, phone_number: '', experience: 0, specialization: '' });
                newUser.doctor_data = relatedDocument._id;
                break;
            case 'School':
                relatedDocument = await School.create({ email, password: hashedPassword, name, camp_ids: [] });
